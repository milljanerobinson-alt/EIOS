/*
# EWO-042: Read-Only Execution Package Inspection RPC

## Purpose
Provides a governed, read-only inspection of a pending execution package.
Returns execution, repository, plan, and governance details without
changing any lifecycle state.

This function NEVER:
- Changes lifecycle state
- Creates execution requests
- Cancels/rejects/approves execution
- Invokes an execution provider
- Creates git branches or commits
...
BEGIN
  ...
  ELSE
    -- Try to find the latest package for this EWO directly
    SELECT p.* INTO v_pkg
    FROM supervised_execution_packages p
