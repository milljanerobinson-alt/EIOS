/*
# EWO-042S.3: Add ref_correction Change Type and Fix Function

## Purpose
1. Adds `ref_correction` to the `engineering_change_types` lookup table.
2. Updates `correct_canonical_ewo_ref()` to use `ref_correction` as the change_type.

## Changes
- New row in `engineering_change_types`: `ref_correction`
- Function updated to use the new change_type
*/

INSERT INTO engineering_change_types (change_type, description)
VALUES ('ref_correction', 'Canonical reference corrected by Product Owner')
...
BEGIN
  ...
  SELECT EXISTS(SELECT 1 FROM engineering_work_orders WHERE ewo_ref = p_corrected_ref) INTO v_corrected_taken;
  IF v_corrected_taken THEN
    RAISE EXCEPTION 'Governed correction violation: corrected ref % is already in use', p_corrected_ref;
  END IF;
  ...
  SET ewo_ref = p_corrected_ref, updated_at = now()
  WHERE id = p_ewo_id;
  ...
  VALUES (
    p_ewo_id, v_status, v_status, p_po_identity,
