/*
# EWO-032R.1 — Fix Gate 4 record-not-assigned error

## Purpose
The Gate 4 SELECT into v_exec_approval is inside an `IF v_ewo IS NOT NULL`
block. When v_ewo is NULL, the SELECT never runs, so v_exec_approval is
never assigned, and `NOT FOUND OR v_exec_approval.decision` raises:
  record "v_exec_approval" is not assigned yet

## Fix
Use an explicit boolean flag `v_approval_found` initialised to false,
set to true only when the SELECT INTO finds a row. Check the flag instead
of `NOT FOUND` in the gate condition.
*/
...
DECLARE
  ...
  v_approval_found boolean := false;
  v_provider record;
...
BEGIN
  ...
  ELSE
    ...
    IF FOUND THEN
      v_approval_found := true;
    END IF;
  END IF;

  IF NOT v_approval_found OR v_exec_approval.decision IS DISTINCT FROM 'approved' THEN
    v_blockers := v_blockers || jsonb_build_object(
  ...
  v_governance_diagnostics := jsonb_build_object(
    ...
    'execution_approval', CASE WHEN v_approval_found THEN v_exec_approval.decision ELSE 'not_found' END,
