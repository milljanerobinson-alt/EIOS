/*
# EWO-042S.3: Fix Correction RPC — Preserve Append-Only Change Log

## Purpose
Updates `correct_canonical_ewo_ref()` to NOT modify `engineering_change_log`
(which is append-only and protected by trigger). Instead:
- Preserves the original change log entry with `ewo_ref = 'EWO-222'`
- Appends a NEW change log entry recording the correction
- Also skips updating `ewo_creation_attempt_log` (also append-only audit)

## Rationale
Both `engineering_change_log` and `ewo_creation_attempt_log` are append-only
audit tables. Modifying them would violate audit integrity. The alias table
and the new change log entry provide forward resolution.

## Changes
- `correct_canonical_ewo_ref()` no longer updates engineering_change_log or
  ewo_creation_attempt_log in place.
- Instead, it inserts a NEW engineering_change_log entry recording the
  correction with `change_type = 'ref_correction'`.
*/
...
BEGIN
  ...
  INSERT INTO engineering_change_log (ewo_ref, change_type, summary, actor)
  VALUES (
    p_corrected_ref,
    'ref_correction',
