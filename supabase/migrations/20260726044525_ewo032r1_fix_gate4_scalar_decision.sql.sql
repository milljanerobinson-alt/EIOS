/*
# EWO-032R.1 — Fix Gate 4 short-circuit evaluation

## Purpose
PL/pgSQL evaluates both sides of `OR`, so `v_exec_approval.decision` is
accessed even when v_approval_found is false, raising:
  record "v_exec_approval" is not assigned yet

## Fix
Store the decision in a scalar text variable `v_approval_decision`
initialised to null. Only reference the scalar in the gate condition.
*/
...
DECLARE
  v_ewo record;
  v_approval_decision text := null;
  v_approval_product_owner text := null;
  v_provider record;
...
BEGIN
  ...
  ELSE
    ...
    SELECT ea.decision, ea.product_owner
      INTO v_approval_decision, v_approval_product_owner
  ...
  IF v_approval_decision IS DISTINCT FROM 'approved' THEN
    v_blockers := v_blockers || jsonb_build_object(
  ...
  v_governance_diagnostics := jsonb_build_object(
    ...
    'execution_approval', COALESCE(v_approval_decision, 'not_found'),
