// EWO-044R2 Acceptance Test Runner (CommonJS for Node.js)
// Uses PRODUCTION tool registry and PRODUCTION tool execution via the edge function.
// No embedded tool definitions, no direct database queries.
// Usage: node scripts/ewo044r2-run-acceptance.cjs
...
async function runTest(testName, prompt, conversationId, accessToken, expectToolCall, expectedTool, resolvedContext) {
  const expectedTools = Array.isArray(expectedTool) ? expectedTool : (expectedTool ? [expectedTool] : []);
  ...
  if (!toolsResp.ok) {
    return { test: testName, prompt, provider: '', nativeToolCalls: [], nativeCallIds: [], executedTools: [], resolvedContext, toolResults: [], toolRounds: 0, finalResponse: '', auditRef: '', passed: false, notes: 'Failed to fetch production tools: ' + toolsResp.status };
  }
  ...
  let allNativeCallIds = [];
  let allNativeToolCalls = [];
  let allToolResults = [];
  let finalContent = '';
  let provider = '';
  ...
  while (loopCount < maxLoops) {
    ...
    if (!resp.ok) {
      ...
      return { test: testName, prompt, provider: '', nativeToolCalls: [], nativeCallIds: [], executedTools: [], resolvedContext, toolResults: [], toolRounds: 0, finalResponse: '', auditRef: '', passed: false, notes: 'Provider call failed: ' + resp.status + ' ' + errText.slice(0, 200) };
    }
  ...
  const { data: auditRecord } = await supabase
    ...
    .limit(1)
    .maybeSingle();
  if (auditRecord) auditRef = auditRecord.audit_reference;

  let passed = false;
  let notes = '';
  if (!expectToolCall) {
    passed = true;
    notes = 'Greeting — no tool call required';
  } else {
    const calledExpected = expectedTools.length === 0 || expectedTools.some(et => allNativeToolCalls.includes(et) || allExecutedTools.includes(et));
    passed = calledExpected;
    notes = calledExpected
      ? 'Provider called expected tool: ' + expectedTools.filter(et => allNativeToolCalls.includes(et) || allExecutedTools.includes(et)).join(', ')
  ...
  return {
    ...
    prompt,
    provider,
    nativeToolCalls: allNativeToolCalls,
    nativeCallIds: allNativeCallIds,
    executedTools: allExecutedTools,
    resolvedContext,
    toolResults: allToolResults,
    toolRounds: loopCount,
    finalResponse: finalContent.slice(0, 500),
    auditRef,
    passed,
    notes,
  };
...
async function main() {
  ...
  console.log('Context resolution:', ctxResp.status);
  let resolvedContext = {};
  if (ctxResp.ok) {
    const ctxData = await ctxResp.json();
    resolvedContext = ctxData.context ?? {};
  ...
  const toolsData = await toolsResp.json();
  const hasListActiveEwos = (toolsData.tools ?? []).some(t => t.name === 'eios_list_active_ewos');
  ...
  const results = [];
  results.push(await runTest('Test 1: Greeting', 'Good morning ATD.', conversationId, accessToken, false, null, resolvedContext));
  ...
  console.log('\nDetailed results: /tmp/ewo044r2-acceptance-results.json');
